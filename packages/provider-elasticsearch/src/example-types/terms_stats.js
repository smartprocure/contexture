...
module.exports = {
    validContext: context => context.config.key_field && context.config.value_field,
    result: function(context, search, schema, provider, options) {
        var filter = null;

        if (context.config.filter) {
            var filterParts = context.config.filter.trim().replace(/\s\s+/g, ' ').toLowerCase().split(' ');
            var rawFieldName = (_.get( 'name', context.config.key_field) || context.config.key_field).replace('.untouched', '.lowercased');
            filter = {
                bool: {
                    must: _.map(f => ({
                        wildcard: {
            ...
            config: {
                ...
                key_data: {
                    ...
                    order: {
                        ...
                        //  order
                        ['twoLevelAgg.' + (context.config.order || 'sum')]: context.config.sortDir || 'desc'
                    }
                },
                value_type: 'stats'
            }
        }, context), search).then(x => ({
            terms: x.results
        }));
    }
};
