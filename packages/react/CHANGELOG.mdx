### 2.40.0
* MemoryTable: updating memory service with fresh data on render
* ResultTable: hiding footer with `hideFooter` prop, applying `cellProps` and `headerCellProps` from field schema
