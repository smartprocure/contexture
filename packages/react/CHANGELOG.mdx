### 2.51.2
* NestedPicker: add support for creating a Commonly Used Fields section if one or more field options have flag `isCommonlyUsed` set (unless opt-out by using prop `makeCommonlyUsedOptionSection`)
* NestedPicker: auto rollup field path(s) to the next group/parent if the field is a single sibling in current field group
