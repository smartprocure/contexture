...
export let getRelevantFilters = _.curry((groupCombinator, Path, firstPath, group) => {
  ...
  let relevantFilters = _.compact(
    _.map(getRelevantFilters(groupCombinator, path, firstPath), relevantChildren)
  )
  ...
  if(firstPath.slice(1).join('.') === path.join('.')){
    return {filters: groupCombinator(group, _.compact(filterArray)), filterNode: _.flow(_.flattenDeep, _.compact)(nodeArray)}
  }else{
    return {filter: groupCombinator(group, _.compact(filterArray)), filterNode: nodeArray}
  }
})
