...
| indentReplace | `async (path, newNode) -> await searchCompleted` | Indents the node at the provided path by replacing it with a group described by `newNode` with the target node as its only child. You can await this for when updates settle and relevant searches are completed. **NOTE:** This should rarely be used directly and is mostly an internal utility used by `indent`. You probably want `indent` which has the same signature. |
| indentInPlace | `async (path, newNode) -> await searchCompleted` | Indents the node at the provided path by mutating it into the node described by `newNode` with the original target node as its only child. You can await this for when updates settle and relevant searches are completed. This is most useful for indenting the root node of a tree when there is no parent (otherwise, `indentInPlace` is a simpler model). **NOTE:** This should rarely be used directly and is mostly an internal utility used by `indent`. You probably want `indent` which has the same signature. |
